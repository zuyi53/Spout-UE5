...
static ID3D12Device* g_D3D12Device;
...
void DestroyTexture(UTexture2D*& Texture)
{
	...
	if (Texture){
		...
		UE_LOG(LogTemp, Warning, TEXT("Texture is ready"));
	}
}
...
void ResetMatInstance(UTexture2D*& Texture, UMaterialInstanceDynamic*& MaterialInstance)
{
	...
	{
		UE_LOG(LogTemp, Warning, TEXT("UUUUUUUUUUUUUUUUUUUUUUUUUUUUUUUUUU"));
		return;
	}
	...
	if (!MaterialInstance)
	{
		...
		if (!MaterialInstance)
		{
			UE_LOG(LogTemp, Warning, TEXT("Material instance can't be created"));
			return;
		}
	}
	...
	if (!MaterialInstance)
	{
		UE_LOG(LogTemp, Error, TEXT("Material instance wasn't created"));
		return;
	}
	...
	{
		UE_LOG(LogTemp, Warning, TEXT("UI Material instance Texture parameter not found"));
		return;
	}
...
void initSpout()
{
	UE_LOG(LogTemp, Warning, TEXT("-----------> Init Spout"));
...
void GetDevice()
{
	UE_LOG(LogTemp, Warning, TEXT("-----------> Set Graphics Device D3D12"));

	g_D3D12Device = (ID3D12Device*)GDynamicRHI->RHIGetNativeDevice();
	//D3D12CreateDevice()

	g_D3D12Device->GetImmediateContext(&g_pImmediateContext);
}
...
FSenderStruct* RegisterReceiver(FName spoutName){
	...
	UE_LOG(LogTemp, Warning, TEXT("--starting...--___Open Shared Resource___---"));
	HRESULT openResult = g_D3D12Device->OpenSharedHandle(newFSenderStruc->sHandle, __uuidof(ID3D12Resource), (void**)(&newFSenderStruc->sharedResource));

	if (FAILED(openResult)) {
		UE_LOG(LogTemp, Error, TEXT("--FAIL--___Open Shared Resource___---"));
		return false;
	...
	description.Usage = D3D12_USAGE_STAGING;
	description.BindFlags = 0;
	description.CPUAccessFlags = D3D12_CPU_ACCESS_READ | D3D12_CPU_ACCESS_WRITE;
	description.MiscFlags = 0;
	description.MipLevels = 1;
	description.ArraySize = 1;


	UE_LOG(LogTemp, Warning, TEXT("--- Creating D3D12 Texture 2D---"));

	HRESULT hr = g_D3D12Device->CreateTexture2D(&description, NULL, &newFSenderStruc->texTemp);
	...
	if (FAILED(hr))
	{
		...
		UE_LOG(LogTemp, Error, TEXT("Failed to create texture of name: ----> %s"), *FString(TestString.c_str()));

		if (hr == E_OUTOFMEMORY) {
			UE_LOG(LogTemp, Error, TEXT("OUT OF MEMORY"));
		}
		...
		UE_LOG(LogTemp, Error, TEXT("Error creating temporal textura"));
		return false;
...
bool USpoutBPFunctionLibrary::SpoutInfoFrom(FName spoutName, FSenderStruct& SenderStruct){
	...
	if (EncontradoSenderStruct == nullptr){
		UE_LOG(LogTemp, Warning, TEXT("No Sender was found with the name : %s"), *spoutName.GetPlainNameString());
		return false;
	}
	else
	{
...
bool USpoutBPFunctionLibrary::CreateRegisterSender(FName spoutName, ID3D12Texture2D* baseTexture)
{
	...
	if (g_D3D12Device == nullptr || g_pImmediateContext == NULL){
		UE_LOG(LogTemp, Warning, TEXT("Getting Device..."));
		GetDevice();
	}
	...
	ID3D12Texture2D * sendingTexture;

	UE_LOG(LogTemp, Warning, TEXT("ID3D12Texture2D Info : width_%i, height_%i"), desc.Width, desc.Height);
	UE_LOG(LogTemp, Warning, TEXT("ID3D12Texture2D Info : Format is %i"), int(desc.Format));
	...
	texResult = sdx->CreateSharedDX11Texture(g_D3D12Device, desc.Width, desc.Height, texFormat, &sendingTexture, sharedSendingHandle);
	UE_LOG(LogTemp, Warning, TEXT("Create shared Texture with SDX : %i"), texResult);

	if (!texResult)
	{
		UE_LOG(LogTemp, Error, TEXT("SharedDX11Texture creation failed"));
		return 0;
	}

	const auto tmp = spoutName.GetPlainNameString();
	UE_LOG(LogTemp, Warning, TEXT("Created Sender: name --> %s"), *tmp);

	//
	senderResult = sender->CreateSender(TCHAR_TO_ANSI(*spoutName.ToString()), desc.Width, desc.Height, sharedSendingHandle, texFormat);
	UE_LOG(LogTemp, Warning, TEXT("Created sender DX11 with sender name : %s"), *tmp);
	...
	// add new register
	UE_LOG(LogTemp, Warning, TEXT("Adding Sender to Sender list"));
	FSenderStruct* newFSenderStruc = new FSenderStruct();
...
ESpoutState CheckSenderState(FName spoutName){
	...
	if (sender->FindSenderName(TCHAR_TO_ANSI(*spoutName.ToString()))) {
		//UE_LOG(LogTemp, Warning, TEXT("Sender State: --> Exist"));
		if (bIsInListSenders) {
			//UE_LOG(LogTemp, Warning, TEXT("Sender State: --> Exist y Registred"));
			state = ESpoutState::ER;
		}
		else {
			//UE_LOG(LogTemp, Warning, TEXT("Sender State: --> Exist y No Registred"));
			state = ESpoutState::EnoR;
		}
	}
	else {
		//UE_LOG(LogTemp, Warning, TEXT("Sender State: --> No Exist"));
		if (bIsInListSenders) {
			//UE_LOG(LogTemp, Warning, TEXT("Sender State: --> No Exist y Registred"));
			state = ESpoutState::noER;
		}
		else {
			//UE_LOG(LogTemp, Warning, TEXT("Sender State: --> No Exist y No Registred"));
...
bool USpoutBPFunctionLibrary::SpoutSender(FName spoutName, ESpoutSendTextureFrom sendTextureFrom, UTextureRenderTarget2D* textureRenderTarget2D, float targetGamma)
{
	...
	ID3D12Texture2D* baseTexture = 0;
	FSenderStruct* SenderStruct = 0;
	
	switch (sendTextureFrom)
	{
	case ESpoutSendTextureFrom::GameViewport:
		baseTexture = (ID3D12Texture2D*)GEngine->GameViewport->Viewport->GetRenderTargetTexture()->GetNativeResource();
		break;
	case ESpoutSendTextureFrom::TextureRenderTarget2D:
		if (textureRenderTarget2D == nullptr) {
			UE_LOG(LogTemp, Warning, TEXT("No TextureRenderTarget2D Selected!!"));
			return false;
		}
		textureRenderTarget2D->TargetGamma = targetGamma;
		baseTexture = (ID3D12Texture2D*)textureRenderTarget2D->GetResource()->TextureRHI->GetTexture2D()->GetNativeResource();
		break;
	default:
		break;
	}
	...
	if (baseTexture == nullptr) {
		UE_LOG(LogTemp, Warning, TEXT("baseTexture is null"));
		return false;
	}
	...
	if (state == ESpoutState::noEnoR || state == ESpoutState::noER) {
		UE_LOG(LogTemp, Warning, TEXT("Creating and registering new Sender..."));
		CreateRegisterSender(spoutName, baseTexture);
		return false;
	}
	if (state == ESpoutState::EnoR) {
		UE_LOG(LogTemp, Warning, TEXT("A Sender with the name %s already exists"), *spoutName.GetPlainNameString());
		return false;
	}
	if (state == ESpoutState::ER) {
		...
		if (SenderStruct->spoutType == ESpoutType::Sender) {
			...
			if (td.Width != SenderStruct->w || td.Height != SenderStruct->h) {
				UE_LOG(LogTemp, Warning, TEXT("Texture Size has changed, Updating registered spout: "), *spoutName.GetPlainNameString());
				UpdateRegisteredSpout(spoutName, baseTexture);
				return false;
			}
		}
		if (SenderStruct->spoutType == ESpoutType::Receiver) {
			UE_LOG(LogTemp, Warning, TEXT("A Sender with the name %s already exists, and you have a receiver in Unreal that is receiving"), *spoutName.GetPlainNameString());
	...
	{
		UE_LOG(LogTemp, Warning, TEXT("activeTextures is null"));
		return false;
	}

	HANDLE targetHandle = SenderStruct->sHandle;

	ID3D12Texture2D * targetTex = SenderStruct->activeTextures;

	if (targetTex == nullptr){
		UE_LOG(LogTemp, Warning, TEXT("targetTex is null"));
		return false;
	}
...
bool USpoutBPFunctionLibrary::SpoutReceiver(const FName spoutName, UMaterialInstanceDynamic*& mat, UTexture2D*& texture)
{
	...
	if (state == ESpoutState::noEnoR) {
		UE_LOG(LogTemp, Warning, TEXT("No sender found registered with the name %s"), *spoutName.GetPlainNameString());
		UE_LOG(LogTemp, Warning, TEXT("Try to rename it, or resend %s"), *SenderNameString);
		return false;
	}
		
	if(state == ESpoutState::noER) {
		UE_LOG(LogTemp, Warning, TEXT("Why are you registered??, unregister, best to close it"));
	...
	if (state == ESpoutState::EnoR) {
		UE_LOG(LogTemp, Warning, TEXT("Sender %s found, registering, receiving..."), *spoutName.GetPlainNameString());
	...
	if (state == ESpoutState::ER) {
		...
		if (SenderStruct->spoutType == ESpoutType::Sender) {
			//UE_LOG(LogTemp, Warning, TEXT("Receiving from sender inside ue4 with the name %s"), *spoutName.GetPlainNameString());
		}

		if (SenderStruct->spoutType == ESpoutType::Receiver) {
			//UE_LOG(LogTemp, Warning, TEXT("Continue Receiver with the name %s"), *SenderName.GetPlainNameString());
			...
			ENQUEUE_RENDER_COMMAND(void)(
				...
				{
					ID3D12Texture2D* t_texTemp = SenderStruct->texTemp;
					ID3D12Texture2D* t_tex = (ID3D12Texture2D*)SenderStruct->sharedResource;
					...
					// with CPU read permissions (D3D12_MAP_READ)
					HRESULT hr = g_pImmediateContext->Map(t_texTemp, 0, D3D12_MAP_READ, 0, &mapped);
					if (FAILED(hr))
					{
						t_texTemp->Release();
						return;
...
void USpoutBPFunctionLibrary::CloseSender(FName spoutName)
{
	...
	UE_LOG(LogTemp, Warning, TEXT("Closing... sender %s"), *spoutName.GetPlainNameString());
	...
	if (state == ESpoutState::noEnoR) {
		UE_LOG(LogTemp, Warning, TEXT("%s is already closed; there is nothing to close!! Please check the name."), *spoutName.GetPlainNameString());
		//return;
	}
	if (state == ESpoutState::noER) {
	...
	if (state == ESpoutState::EnoR) {
			UE_LOG(LogTemp, Warning, TEXT("A Sender with the name %s already exists. You can not close a sender that is not yours??"), *spoutName.GetPlainNameString());	
		//return;
	}
	if (state == ESpoutState::ER) {
		...
		if (tempSenderStruct->spoutType == ESpoutType::Sender) {
			UE_LOG(LogTemp, Warning, TEXT("Releasing sender %s"), *spoutName.GetPlainNameString());
			// here really release the sender
			sender->ReleaseSenderName(TCHAR_TO_ANSI(*spoutName.ToString()));
			UE_LOG(LogTemp, Warning, TEXT("Sender %s released"), *spoutName.GetPlainNameString());
			
		}
		else {
			UE_LOG(LogTemp, Warning, TEXT("Receiver always listening"));

			FlushRenderingCommands();
			if (tempSenderStruct->sharedResource != nullptr) {
				UE_LOG(LogTemp, Warning, TEXT("Release sharedResource"));
				tempSenderStruct->sharedResource->Release();
			}
			if (tempSenderStruct->texTemp != nullptr) {
				UE_LOG(LogTemp, Warning, TEXT("Release Temporal texTemp"));
				tempSenderStruct->texTemp->Release();
			}
			if (tempSenderStruct->rView != nullptr) {
				UE_LOG(LogTemp, Warning, TEXT("Release rView"));
				tempSenderStruct->rView->Release();
			}
			UE_LOG(LogTemp, Warning, TEXT("Released All Temporal Textures"));
			//return;
		}
	...
	UE_LOG(LogTemp, Warning, TEXT("There are now %i senders remaining"), FSenders.Num());
	

}


bool USpoutBPFunctionLibrary::UpdateRegisteredSpout(FName spoutName, ID3D12Texture2D * baseTexture)
{
	HANDLE sharedSendingHandle = NULL;
	bool texResult = false;
	bool updateResult = false;
	...
	UE_LOG(LogTemp, Warning, TEXT("ID3D12Texture2D Info : ancho_%i, alto_%i"), desc.Width, desc.Height);
	UE_LOG(LogTemp, Warning, TEXT("ID3D12Texture2D Info : Format is %i"), int(desc.Format));
	...
	// update register
	UE_LOG(LogTemp, Warning, TEXT("Updating Sender in Sender list"));
